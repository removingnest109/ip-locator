use actix_web::{get, web, web::Data, App, HttpResponse, Responder};
use reqwest::Client;
...
async fn search_ip(ip: web::Query<HashMap<String, String>>, tera: Data<Tera>) -> impl Responder {
    ...
    let client = Client::new();
    let response = client
        .get(format!("https://ipinfo.io/{}/json", &ip_address))
        .send()
        .await
        .unwrap()
        .text()
    ...
    HttpResponse::Ok().body(
        tera.render("components/search_results.html", &search_ip_context)
            .unwrap(),
    )
}
